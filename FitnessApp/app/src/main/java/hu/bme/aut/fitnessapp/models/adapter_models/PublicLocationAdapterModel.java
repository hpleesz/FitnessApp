...
public class PublicLocationAdapterModel {
    ...
    public String gymOpenText(int position) {
        ...
        String open = items.get(position).getOpenHours().get(day)[0].replace(":", "");
        open = open.replaceAll("^0+", "");
        if(open.equals("")) openTime = 0;
        else openTime = Integer.parseInt(open);

        String close = items.get(position).getOpenHours().get(day)[1].replace(":", "");
        ...
        if(time >= openTime && time <= closeTime) textView = "Open" + "(-" + items.get(position).getOpenHours().get(day)[1] + ")";
