...
import hu.bme.aut.fitnessapp.models.user_models.measurement_models.MeasurementsGraphFragmentModel;
...
public class MeasurementsGraphFragment extends Fragment implements com.github.mikephil.charting.listener.OnChartValueSelectedListener, MeasurementsGraphFragmentModel.ListLoadedListener
{
    ...
    public void initializeLayout(View rootView) {
        TextView title = rootView.findViewById(R.id.measurementsEntries);
        String text = bodyPart + " " + getString(R.string.entries);
        title.setText(text.replace('_', ' '));

        context = getContext();

        chart = rootView.findViewById(R.id.chartMeasurement);

        recyclerView = rootView.findViewById(R.id.MeasurementRecyclerView);
    ...
    private void updatechart(boolean drawvalues) {
